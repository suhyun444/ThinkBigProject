...
public class RecognizeDigitsAI : MonoBehaviour
{
    ...
    {
        float[] modelInputs = drawingCalculator.CalculateDrawing(new int[2] { 28, 28 });
        ...
        sw.Start();
        solver.Evaluate(mnist_FC_net, modelInputsFormatted, Noedify_Solver.SolverMethod.MainThread);
        sw.Stop();
        ...
        float curPredictionPercent = 0;
        for (int i = 0; i < predictions.Length; i++)
        {
            predictions_log[i] = (Mathf.Log10(predictions[i]) + 5f) / 5f;
            if(curPredictionPercent < predictions_log[i])
            {
                curPredictionPercent = predictions_log[i];
                predictionNumber = i;
            }
        }
        //Debug.Log(predictionNumber);
        curPredictNumber = predictionNumber;
        return predictionNumber;
    }

    public void Commit(int predictionNumber)
    {
        monster.CheckAnswer(predictionNumber);
    }
